...
| MapsV1 | `com.google.android.maps` | framework | Shared-lib JAR + permissions XML |
...
(Android build-tools), `zip`, and a sha256 tool; `xmllint` is optional. Until a
bump pins real hashes, every `sha256` is the sentinel `PENDING-BUMP` and the
build refuses to run -- by design.
